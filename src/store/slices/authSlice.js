...
export const userLogin = createAsyncThunk('auth/userLogin', async ({ username, password, Comid = '1' }, { rejectWithValue }) => {
  try {
    const rows = asArray(unwrap(await api.login({ username, password, Comid })));
    const user = rows[0];
