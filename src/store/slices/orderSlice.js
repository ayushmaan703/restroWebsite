  ...
  async ({ tableId, Comid = "1" } = {}, { rejectWithValue }) => {
    try {
      const summaries = asArray(
        unwrap(await api.getTableOrders({ Comid, typ: 4 })),
      ).map(normalizeOrderSummary);
      const target = summaries.find(
        (order) => String(order.tableId) === String(tableId),
      );
      ...
      const items = asArray(
        unwrap(await api.getOrderDetail({ transid: target.transid, Comid })),
      ).map(normalizeOrderItem);
      return { ...target, items };
    } catch (error) {
      return rejectWithValue(
...
export const deleteOrderItem = createAsyncThunk(
  ...
  async (
    { trans3id, Uid = 1, Comid = "1", transid } = {},
    { rejectWithValue },
  ) => {
    try {
      ...
      const items =
        transid && String(transid) !== "0"
          ? asArray(unwrap(await api.getOrderDetail({ transid, Comid }))).map(
              normalizeOrderItem,
            )
          : [];
...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchRunningOrderForTable.pending, (state) => {
        state.error = null;
      })
...
export const { setCurrentOrder, decrementOrderItem, resetCurrentOrder } =
  slice.actions;
