...
export default function CustomerCart() {
  ...
  const tableId =
    params.get('table') ||
    params.get('TableId') ||
    sessionStorage.getItem('customerTable') ||
    '';
  const Comid =
    params.get('Comid') ||
    sessionStorage.getItem('customerComid') ||
    '1';
    ...
    <div className="customer-page">
      <div className="customer-card cart-card">
        <button
          ...
          onClick={() =>
            navigate(
              `/customer/order?table=${encodeURIComponent(
                tableId,
            ...
            {items.map(item => (
              <div className="cart-line" key={item.id}>
                ...
                <div className="cart-controls">
                  <button
                    onClick={() =>
                      dispatch(changeQty({ id: item.id, delta: -1 }))
                    }
        ...
        {items.length > 0 && (
          <button
            ...
            onClick={() =>
              navigate(
                `/customer/checkout?table=${encodeURIComponent(
                  tableId,
                )}&Comid=${encodeURIComponent(Comid)}`,
              )
            }
