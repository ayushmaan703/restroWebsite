...
export const normalizeTable = (row, index) => {
  const availability = String(
    value(row, ["Avilability", "Availability", "availability"], ""),
  ).toUpperCase();
  ...
  return {
    ...
    tableNo: value(
      row,
      ["TableNo", "TableName", "Table", "Tableno", "tableNo"],
      `Table ${index + 1}`,
    ),
...
export const normalizeCategory = (row, index) => ({
  id: String(
    value(row, ["CategoryId", "CategoryID", "MenuCategoryId", "id"], index),
  ),
  name: value(
    row,
...
export const normalizeMenuItem = (row, index) => ({
  id: String(
    value(row, ["ItemId", "ItemID", "MenuId", "MenuID", "id"], index + 1),
  ),
  name: value(
    row,
    ["Item", "ItemName", "MenuName", "Menu", "Name", "menuName"],
    `Item ${index + 1}`,
  ),
  price: toNumber(
    value(row, ["SellPriceWithGst", "SellPrice", "Rate", "Price", "Amount"], 0),
  ),
  priceWithoutGst: toNumber(
    value(row, ["SellPrice", "Amount_WithoutGst", "Rate", "Price"], 0),
  ),
  gst: toNumber(value(row, ["GstAmt", "GST", "Gst"], 0)),
  category: value(row, ["Category"], ""),
  categoryId: String(
    value(row, ["CategoryId", "CategoryID", "categoryId"], "0"),
  ),
...
export const normalizeOrderItem = (row, index) => ({
  key: String(
    value(
      row,
      ["trans3id", "Trans3Id", "id", "Id", "DetailId", "OrderDetailId"],
      index,
    ),
  ),
  ...
  price: toNumber(
    value(row, ["Amount", "SellPriceWithGst", "Rate", "Price"], 0),
  ),
  priceWithoutGst: toNumber(
    value(row, ["Amount_WithoutGst", "SellPrice", "Rate", "Price"], 0),
  ),
  gst: toNumber(value(row, ["GstAmt", "GST", "Gst"], 0)),
